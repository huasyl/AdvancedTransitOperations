...
namespace RapidTransitMod.RailEtaHost
{
    internal static class RailEtaLimits
    {
        ...
        public const int MaxActiveSnapshotBatches = 1;
        public const int MaxPredictorQueue = 8;
        ...
        public const int MaxPropagationRounds = 32;
        ...
        public const int MaxTrajectorySamples = 4096;
    ...
    {
        public RailEtaRequestDescriptor(int vehicleIndex, int vehicleVersion, long targetCheckpointId)
        {
    ...
    public sealed class RailEtaTicketStatus
    {
        ...
        public uint ScopedScheduledFrame { get; internal set; }
        ...
        public uint DepartureFrame { get; internal set; }
        public uint RemainingHoldFrames { get; internal set; }
    ...
    internal sealed class RailEtaTicketStore
    {
        ...
        public void CompleteHold(RailEtaTicket ticket, uint originFrame, uint departureFrame, uint remainingFrames)
        {
            if (!m_Entries.TryGetValue(ticket.Value, out Entry entry)) return;
            lock (entry.Gate)
            {
                entry.Status.State = RailEtaRequestState.Completed;
                entry.Status.Failure = RailEtaFailure.StoppedByDispatchHold;
                entry.Status.OriginFrame = originFrame;
                entry.Status.DepartureFrame = departureFrame;
                entry.Status.RemainingHoldFrames = remainingFrames;
        ...
        private static RailEtaTicketStatus Copy(RailEtaTicketStatus source) => new RailEtaTicketStatus
        {
            ...
            ScopedScheduledFrame = source.ScopedScheduledFrame,
            ...
            RemainingHoldFrames = source.RemainingHoldFrames,
        ...
        private static void ApplyStageFrame(RailEtaTicketStatus status, RailEtaRequestState state, uint frame)
        {
            switch (state)
            {
                ...
                case RailEtaRequestState.ScopedJobScheduled: status.ScopedScheduledFrame = frame; break;
