...
namespace RapidTransitMod
{
    public partial class DispatchRuntimeSystem
    {
        ...
        {
            ClearAssistLaunchPending(v);
            m_RuntimeController.ClearBoardingGrace(v);
            ...
            EntityCommandBuffer ecb)
        {
            ...
            m_RuntimeController.SetPreparing(v, nowFrame);
            m_CachedWpIdx[v] = -1;
        ...
        private void ReleaseRuntimeOwnershipAfterRetireHandoff(Entity vehicle, string reason)
        {
            ...
            m_LapObservations.Remove(vehicle);
        ...
        internal void GuardRetireHandoffDispatchInputs(uint nowFrame)
        {
            ...
            foreach (Entity vehicle in watchedVehicles)
            {
                ...
                bool handoffBoundaryReady = IsRetireHandoffRouteBoundaryReady(vehicle, headVehicle, pathEndReached, out string handoffBoundary);
                ...
                bool officialDepotReturning = (publicWasReturning || cargoWasReturning)
                    && alreadyDepotTarget;

                bool accelerateOfficialBoardingClose = wasBoarding && handoffBoundaryReady && targetWasRouteWaypoint;
                uint officialBoardingCloseFrame = nowFrame > OFFICIAL_BOARDING_CLOSE_TIMEOUT_FRAMES
                ...
                if (EntityManager.HasComponent<Game.Vehicles.PublicTransport>(vehicle))
                {
                    ...
                    if (!officialDepotReturning)
                    {
                        ...
                        if (accelerateOfficialBoardingClose
                            && (oldState & PublicTransportFlags.Boarding) != 0)
                        {
                ...
                if (EntityManager.HasComponent<Game.Vehicles.CargoTransport>(vehicle))
                {
                    ...
                    if (!officialDepotReturning)
                    {
                        ...
                        if (accelerateOfficialBoardingClose
                            && (oldState & CargoTransportFlags.Boarding) != 0)
                        {
                    ...
                    cargoReturning = true;
                    if (cargoRequestCount != 0
                        || cargoTransport.m_State != oldState
                        || cargoClampedDeparture)
                    {
        ...
        private bool IsRetireHandoffRouteBoundaryReady(
            ...
            out string boundary)
        {
            if (strictPathEndReached
                || HasTrainLaneFlags(headVehicle, TrainLaneFlags.EndOfPath | TrainLaneFlags.EndReached))
            {
        ...
        private void SchedulerTick(EntityCommandBuffer ecb, int nowMin)
        {
            ...
            try
            {
                foreach (var line in lines)
                {
                    ...
                    bool useWorkbenchSchedule = IsDispatchRuntimeManagedLine(line);
                    ...
                    for (int i = 0; i < runtimeVehicles.Count; i++)
                    {
                        ...
                        if (m_LapObservations.TryFrames(v0, out uint lf0) && lf0 > 0)
                        {
                            lineHasHistory = true;
                            break;
                        }
                    ...
                    float lineDurationFrames = 0f;
                    {
                        ...
                        {
                            Entity v0 = runtimeVehicles[i];
                            if (m_LapObservations.TryFrames(v0, out uint lf0) && lf0 > maxLapFrames)
                                maxLapFrames = lf0;
                        }
                        if (maxLapFrames > 0)
                        {
                        ...
                        else
                        {
                            if (cachedLapFrames > 0f)
                            {
                                ...
                                for (int i = 0; i < runtimeVehicles.Count; i++)
                                {
                                    Entity v0 = runtimeVehicles[i];
                                    if (!m_LapObservations.TryFrames(v0, out uint lf0) || lf0 == 0)
                                        m_LapObservations.SetFrames(v0, (uint)cachedLapFrames);
                                }
                            }
                            else
                            {
                        ...
                        if (bestVehicle != Entity.Null)
                        {
                            ...
                            if (bst == VehicleState.Idle || bst == VehicleState.Holding)
                            {
                                ...
                                if (currentHolder != Entity.Null)
                                    m_RuntimeController.ReleaseTarget(currentHolder);
                            ...
                            else
                            {
                                if (currentHolder != Entity.Null)
                                    m_RuntimeController.ReleaseTarget(currentHolder);
                                m_RuntimeController.Target(bestVehicle, slot);
        ...
        {
            m_RuntimeController.Target(v, slot);
            var pt = EntityManager.GetComponentData<Game.Vehicles.PublicTransport>(v);
