...
{
    internal static class FeatureSettingsBuffer
    {
        internal static void Ensure(EntityManager entityManager, Entity city)
        {
            if (city != Entity.Null && !entityManager.HasBuffer<OverviewFeatureSettingsStateElement>(city))
            {
                entityManager.AddBuffer<OverviewFeatureSettingsStateElement>(city);
            }
        }

        internal static string Read(DynamicBuffer<OverviewFeatureSettingsStateElement> buffer)
        {
            if (buffer.Length == 0)
            {
                return string.Empty;
            ...
            OverviewFeatureSettingsStateElement[] ordered = new OverviewFeatureSettingsStateElement[buffer.Length];
        ...
        internal static void Write(
            DynamicBuffer<OverviewFeatureSettingsStateElement> buffer,
            List<string> chunks)
        {
            if (buffer.Length > 0)
            {
            ...
            {
                buffer.Add(new OverviewFeatureSettingsStateElement
                {
                    m_ChunkIndex = i,
        ...
        private static string Join(IReadOnlyList<OverviewFeatureSettingsStateElement> chunks)
        {
            if (chunks == null || chunks.Count == 0)
            {
                return string.Empty;
        ...
        private static int Fit(string payload, int offset)
        {
