...
namespace RapidTransitMod.TrackModel
{
    internal sealed partial class TrackModelService
    {
        ...
        private void LogTrackModelReplayDump(NativeArray<Entity> allLines)
        {
            ...
            sb.Append("time=").Append(SlotStr((int)(m_Runtime.FrameIndex / (uint)SimFramesPerMinute) % 1440))
              .Append(" lines=").Append(targetLines.Count)
              .Append(" managedVehicles=").Append(m_Runtime.ManagedVehicleCount)
              .AppendLine();
            ...
            out string row)
        {
            ...
            var allLines = m_Runtime.GetLineEntities(Allocator.Temp);
            try
            {
                for (int i = 0; i < allLines.Length; i++)
                {
