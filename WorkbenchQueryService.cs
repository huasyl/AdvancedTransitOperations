...
{
    internal sealed class WorkbenchQueryService
    {
        private readonly WorkbenchDraftStore m_Drafts;
        private readonly AppliedTimetableStore m_AppliedTimetables;
        private readonly IReadOnlyDictionary<string, AppliedWorkbenchLineState> m_LegacyApplied;
        ...
        private readonly Func<int, string> m_MinutesToSlotString;
        ...
        public WorkbenchQueryService(
            ...
            IReadOnlyDictionary<string, AppliedWorkbenchLineState> legacyApplied,
            ...
            Func<int, string> minutesToSlotString,
            Func<List<DispatchWorkbenchStagedRowDto>, string, bool> hasRowsForDraft)
        {
            ...
            m_LegacyApplied = legacyApplied ?? throw new ArgumentNullException(nameof(legacyApplied));
            ...
            m_MinutesToSlotString = minutesToSlotString ?? throw new ArgumentNullException(nameof(minutesToSlotString));
            ...
            List<DispatchWorkbenchStagedRowDto> rows = m_AppliedTimetables.GetAll(mode)
                ...
                .SelectMany(entry =>
                {
                    ...
                    if (appliedRows.Length == 0)
                    {
                        return (entry.Value?.DepartureMinutes ?? Array.Empty<int>())
                            .Select((minute, index) => new DispatchWorkbenchStagedRowDto
                            {
                                ...
                                lineId = lineId,
                                time = m_MinutesToSlotString(minute),
                    ...
                    return appliedRows.Select((row, index) => new DispatchWorkbenchStagedRowDto
                    {
                        ...
                        lineId = lineId,
                        time = m_MinutesToSlotString(row.DepartureMinute),
                ...
                .ToList();
            if (rows.Count > 0 || m_LegacyApplied.Count == 0)
            {
                return rows.ToArray();
            }

            return m_LegacyApplied
