...
namespace RapidTransitMod
{
    ...
    internal sealed class AnnouncementHost
    {
        ...
        private sealed class RuntimePort : Broadcasting.BroadcastAccess.Host
        {
            ...
            internal override ulong Signature(DynamicBuffer<RouteWaypoint> waypoints) => m_Host.ComputeLineWaypointSignature(waypoints);
            internal override bool TryTurnbacks(LineTrackChain chain, List<TrackTurnbackStationBoundary> stationBoundaries) => m_Host.TryCollectTurnbackStationBoundaries(chain, stationBoundaries);
            ...
            internal override bool TryTurnback(LineTrackChain chain, TurnbackBoundary boundary, out TrackTurnbackStationBoundary stationBoundary) => m_Host.TryResolveTurnbackStationBoundary(chain, boundary, out stationBoundary);
            ...
            internal override float EstimatePreparing(
                ...
                uint nowFrame)
            {
                return m_Host.EstimatePreparingArrivalFrames(vehicle, line, waypoints, nowFrame, LapFrames(line));
            }
